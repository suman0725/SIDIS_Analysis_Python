...
def get_phih(E_beam,
    ...
    qnorm = np.linalg.norm(q3, axis=-1)
    ...
    if np.any(good):
        ...
        cross_qp  = np.cross(qhat, p3)
        # v_had direction ~ (q x ph)
        cross_qph = np.cross(qhat, ph3)
        # For numerator term
        cross_pph = np.cross(p3, ph3)

        # 4. Calculate Terms
        # Y-term (Sine-like): (p x ph) . qhat
        num = np.einsum("...i,...i->...", cross_pph, qhat)
        
        # X-term (Cosine-like): (qhat x p) . (qhat x ph)
        den = np.einsum("...i,...i->...", cross_qp,  cross_qph)
